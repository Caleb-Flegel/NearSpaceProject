...
DigitalOut errorLED(p21); //Signals there was an error
DigitalOut sdLED(p22); //Signals there is data being written to the SD card
DigitalOut compLED(p23); //Signals the program has been sucessfully completed

//Value that converts angular velocity into a speed setting that won't make the motor spin too fast
#define ERROR_CONVERSION .000004
...
int main() {
  ...
  fprintf(logFile, "\n\nPitch\tRoll\tHeading  Motor_Speed  Angular_Velocity\n");
  ...
  while (masterTimer.elapsed_time() < 220min) {
    ...
    fprintf(logFile, "%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t\n", headings[2], headings[1], headings[0], currSpeed, gyros[1]); 
